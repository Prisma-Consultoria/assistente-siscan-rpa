...
Implements Module-Main -Args <hashtable>
#>
...
function Module-Main {
    ...
    $token = $Args.Token
    $siscanUser = $Args.SiscanUser
    $siscanPass = $Args.SiscanPass
    ...
    # Create a minimal docker-compose to run the service
    $compose = @"
version: '3.8'
services:
  assistente-siscan-rpa:
    ...
    environment:
      - SISCAN_USER=$siscanUser
      - SISCAN_PASS=$siscanPass
    volumes:
      - $dataDir:/app/data
    ports:
      - "8080:8080"
