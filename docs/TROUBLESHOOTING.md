...
### Problema 5 — pg_restore falha com "unrecognized configuration parameter transaction_timeout"

Sintoma: restauração de backup falha com:
```
pg_restore: error: could not execute query: ERROR:  unrecognized configuration parameter "transaction_timeout"
Command was: SET transaction_timeout = 0
```

**Causa:** o dump foi gerado com `pg_dump` do PostgreSQL 17, que inclui `SET transaction_timeout = 0`. Se o servidor de destino é PostgreSQL 16 ou anterior, esse parâmetro não existe.

**Solução:** converter o dump para SQL, filtrar a linha problemática e aplicar via `psql`:

```bash
cd /app/assistente-siscan-rpa
docker compose -f docker-compose.prd.rpa.yml exec -T -e PGPASSWORD='SENHA' app \
  pg_restore --file=- --clean --if-exists --no-owner \
  < backups/<nome_do_dump>.dump \
  | grep -v transaction_timeout \
  | docker compose -f docker-compose.prd.rpa.yml exec -T -e PGPASSWORD='SENHA' app \
  psql -h <IP-VM2> -U <USUARIO> -d <BANCO>
```

> Substitua `SENHA`, `<IP-VM2>`, `<USUARIO>` e `<BANCO>` conforme o ambiente. O `pg_restore --file=-` converte o dump binário para SQL no stdout, o `grep -v` remove a linha incompatível, e o `psql` aplica o SQL filtrado.
