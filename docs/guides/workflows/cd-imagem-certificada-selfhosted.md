...
| 2 | Validar runner TLS (`--pre-flight`) | Roda `bash scripts/deploy_server/check-runner-tls.sh --quiet --pre-flight` (step adicionado pós-F00.05). Detecta proativamente falhas de TLS contra `*.actions.githubusercontent.com` antes que o deploy quebre num pull do GHCR — o specialist mais útil pré-deploy, introduzido em [TSK00.04.10](https://github.com/Prisma-Consultoria/assistente-siscan-rpa/pull/93). |
